 ...
 * @param[in] param パラメタ
 */
void Neumann::connectPlayers(const spd::core::AllPlayer& players,
 ...
 */
spd::core::Neighbors Neumann::getNeighbors(const spd::core::AllPlayer& players,
		int target,
		int radius) {
 ...
 */
int Neumann::minStrategyLength(int actionRadius) {
 ...
 * @retval 0 できるかぎり使う場合
 */
int Neumann::maxStrategyLength(int actionRadius) {
	return calcNeighborsNum(actionRadius);
}
 ...
 * @param[in] space 空間
 */
void Neumann::accept(spd::output::OutputVisitor& visitor, spd::core::Space& space) {
 ...
 * 再設定はしない
 */
void Neumann::reSetting(const spd::core::AllPlayer& players,
		const spd::param::Parameter& param) {
	return;
}
 ...
 */
bool Neumann::isCompliant(const std::shared_ptr<spd::param::InitParameter>& iniParam, std::string& errMsg) {
	...
	if (side * side != iniParam->getPlayerNum()) {
		...
		errMsg += " when you simulate on a Neumann.";
		result = false;
	}
 ...
 * 文字出力
 */
std::string Neumann::toString() const {
	return "Neumann";
}
