 ...
 * @param[in] param パラメタ
 */
void Moore::connectPlayers(const spd::core::AllPlayer& players,
 ...
 */
spd::core::Neighbors Moore::getNeighbors(const spd::core::AllPlayer& players,
		int target,
		int radius) {
	...
	for (int y = -1 * radius; y <= radius; ++y) {
		for (int x = -1 * radius; x <= radius; ++x) {
			...
			result->at(std::max(std::abs(y), std::abs(x)))->push_back(players.at(nSite));
		}
	}
 ...
 */
int Moore::minStrategyLength(int actionRadius) {
 ...
 * @retval 0 できるかぎり使う場合
 */
int Moore::maxStrategyLength(int actionRadius) {
	return calcNeighborsNum(actionRadius);
}
 ...
 */
bool Moore::isCompliant(const std::shared_ptr<spd::param::InitParameter>& iniParam, std::string& errMsg) {
	...
	if (side * side != iniParam->getPlayerNum()) {
		...
		errMsg += " when you simulate on a Moore.";
		result = false;
	}
 ...
 * 文字出力
 */
std::string Moore::toString() const {
	return "Moore";
}
